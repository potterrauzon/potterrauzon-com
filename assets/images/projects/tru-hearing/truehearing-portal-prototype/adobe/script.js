class Navigation {
	...
	enableMainNav(){
		for(let i=0; i<this.mainNavItems.length; i++){
			this.mainNavItems[i].element.addEventListener("click", this.mainNavClickHandler);
		}
		/*console.log("ENABLE MAIN NAV");*/
	}
	
	disableMainNav(){
		for(let i=0; i<this.mainNavItems.length; i++){
			this.mainNavItems[i].element.removeEventListener("click", this.mainNavClickHandler);
		}
		/*console.log("DISABLE MAIN NAV");*/
	}
...
class View {
	...
	updateMainNav(event){
		...
		for (let i = 0; i < nav.mainNavItems.length; i++){
			let ele = gsap.utils.selector(nav.mainNavItems[i].element);
			gsap.set(ele(".ni-icon-red"), {autoAlpha:0});
			gsap.set(ele(".ni-icon-gray"), {autoAlpha:1});

			gsap.to(ele(".ni-text-active"), {autoAlpha:0, duration:.5, ease:"expo.inOut"});
			gsap.to(ele(".ni-text"), {autoAlpha:1, duration:.5, ease:"expo.inOut"});
		}
		
		let target = gsap.utils.selector(event.currentTarget);
		...
		gsap.to(nav.activeBar.element, {duration:1, x:event.currentTarget.offsetLeft, ease:"expo.out",onComplete:this.updateMainNavOnComplete});

	}

	updateMainNavOnComplete(){
		nav.enableMainNav()
	}
